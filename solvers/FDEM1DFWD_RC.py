...
def FDEM1DFWD_RC(S, M):

    #
    # Update parameters (optional; i.e. if not included in S structure)
    #

    S['r'] = np.sqrt(S['x']**2 + S['y']**2 + (S['z'] + S['height'])**2)  # Coil spacing (m)
    S['omega'] = 2*np.pi*S['freq']                                         # Angular frequency (Rad/s)
...
def magneticFields(S, M):
    ...
    eps0 = 1/(35950207149.4727056*np.pi)        # Vacuum permittivity (F/m)
    ...
    if S['ori'].upper() == 'ZZ':
        ...
        H  = S['mom']/(4*np.pi) * digitalFilter('rTEp2', 0, S, M, mu0, eps0)
        Hn = S['mom']/(4*np.pi) * digitalFilter('rTE02', 0, S, M, mu0, eps0)
        ...
        H  = -S['mom']/(4*np.pi) * (1/S['r'] - 2*S['x']**2/S['r']**3) * \
              digitalFilter('rTEn1', 1, S, M, mu0, eps0) - \
              S['mom']/(4*np.pi) * S['x']**2/S['r']**2 * \
              digitalFilter('rTEn2', 0, S, M, mu0, eps0)
        Hn = -S['mom']/(4*np.pi) * (1/S['r'] - 2*S['x']**2/S['r']**3) * \
        ...
        H  =  S['mom']/(2*np.pi) * S['x']*S['y']/S['r']**3 * \
              digitalFilter('rTEn1', 1, S, M, mu0, eps0) - \
              S['mom']/(4*np.pi) * S['x']*S['y']/S['r']**2 * \
              digitalFilter('rTEn2', 0, S, M, mu0, eps0)
        Hn = -S['mom']/(4*np.pi) * (1/S['r'] - 2*S['x']**2/S['r']**3) * \
              digitalFilter('rTE01', 1, S, M, mu0, eps0) - \
              S['mom']/(4*np.pi) * S['x']**2/S['r']**2 * \
        ...
        H  =  S['mom']/(2*np.pi) * S['x']*S['y']/S['r']**3 * \
              ...
              S['mom']/(4*np.pi) * S['x']*S['y']/S['r']**2 * \
              digitalFilter('rTEn2', 0, S, M, mu0, eps0)
        Hn = -S['mom']/(4*np.pi) * (1/S['r'] - 2*S['y']**2/S['r']**3) * \
              digitalFilter('rTE01', 1, S, M, mu0, eps0) - \
              S['mom']/(4*np.pi) * S['y']**2/S['r']**2 * \
              digitalFilter('rTE02', 0, S, M, mu0, eps0)
        ...
        H  = -S['mom']/(4*np.pi) * (1/S['r'] - 2*S['y']**2/S['r']**3) * \
              digitalFilter('rTEn1', 1, S, M, mu0, eps0) - \
              S['mom']/(4*np.pi) * S['y']**2/S['r']**2 * \
              digitalFilter('rTEn2', 0, S, M, mu0, eps0)
        Hn = -S['mom']/(4*np.pi) * (1/S['r'] - 2*S['y']**2/S['r']**3) * \
        ...
        raise ValueError('Transmitter-receiver orientation unknown.')
...
def digitalFilter(funname, order, S, M, mu0, eps0):
    ...
    flen = len(filt_w)
    ind = np.arange(1, flen+1)
    l1 = 1.0 / S['r']
    l2 = 10**(filt_a + (ind-1) * filt_s)
    lam = l1 * l2
    ...
    if funname == 'rTE01':
        YF = rTE01(lam, S, mu0, eps0)
    elif funname == 'rTE02':
        YF = rTE02(lam, S, mu0, eps0)
    elif funname == 'rTEn1':
        YF = rTEn1(lam, S, M, mu0, eps0)
    elif funname == 'rTEn2':
        YF = rTEn2(lam, S, M, mu0, eps0)
    elif funname == 'rTEp2':
        YF = rTEp2(lam, S, M, mu0, eps0)

    #
    # Calculate output, considering weights and r
    #

    y = np.dot(YF, filt_w) / S['r']
...
#  S (dict)            Sensor characteristics
#  M (dict)            Model characteristics
#  lam                 Hankel transform parameter
#
#  Based on:
#  Ward, S. H., and G. W. Hohmann, 1987, Electromagnetic theory for
#  geophysical applications, in M. N. Nabighian, ed., Electromagnetic
...
def reflectionCoefficient(lam, S, M, mu0, eps0):
    ...
    u0 = np.sqrt(lam**2 - S['omega']**2 * mu0 * eps0 + 0j)
    u  = np.sqrt(lam[:, np.newaxis]**2
                 - (S['omega']**2 * mu * M['perm'])[np.newaxis, :]
                 + (1j * S['omega'] * mu * M['con'])[np.newaxis, :])

    #
    # Calculate Y and create Yhat
    #

    Y0   = u0 / (1j * S['omega'] * mu0)
    Y    = u  / (1j * S['omega'] * mu)[np.newaxis, :]
    Yhat = Y.copy()
    ...
    for ii in range(len(M['con'])-2, -1, -1):

        #
        # Calculate tanh
        #

        tanh_uh = np.tanh(u[:, ii] * M['thick'][ii])
...
def rTE02(lam, S, mu0, eps0):
    ...
    y = np.exp(-u0 * (S['z'] + S['height'])) * lam**3 / u0
...
# ----------------------------------------------------- Function 3/5 ---- #
###########################################################################
def rTEn1(lam, S, M, mu0, eps0):

    #
    # Calculate rTE
    #

    rTE = reflectionCoefficient(lam, S, M, mu0, eps0)
    ...
    y = (-rTE * np.exp(u0 * (S['z'] - S['height']))) * lam
...
def rTEn2(lam, S, M, mu0, eps0):
    ...
    y = (-rTE * np.exp(u0 * (S['z'] - S['height']))) * lam**2
...
def rTEp2(lam, S, M, mu0, eps0):
    ...
    y = (rTE * np.exp(u0 * (S['z'] - S['height']))) * lam**3 / u0
